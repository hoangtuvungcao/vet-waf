 ...
 *
 * Since multiple Vet-WAF nodes can use the same configuration and share
...
/**
 * Peer TLS configuration. Each virtual server (vhost) inside Vet-WAF
...
typedef bool ttls_tft_limit_conn_cb_t(TlsTft fingerprint);
typedef bool ttls_tft_limit_rec_cb_t(TlsTft fingerprint);
